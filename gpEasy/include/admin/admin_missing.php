...
class admin_missing extends special_missing{
	...
	function ShowRedirection(){
		...
		foreach($this->error_data['redirects'] as $source => $data){
			...
			if( !empty($data['target']) && $this->isGPLink($data['target']) ){
			...
			$this->InlineBoxLink($langmessage['edit'],'updateredir',$raw_source,$data['target'],$data['code']);

			echo ' &nbsp; ';
			echo common::Link($source,$langmessage['Test']);

			echo ' &nbsp; ';
			echo common::Link('Admin_Missing',$langmessage['delete'],'cmd=rmredir&link='.urlencode($source),' name="gpajax"');
			echo '</td>';
			echo '</tr>';
		}
		echo '</tbody>';
	...
	function RedirForm(){
		...
		echo '<tr>';
			...
			echo ' <input type="button" class="admin_box_close" name="" value="'.$langmessage['cancel'].'" />';
			echo '</td></tr>';
