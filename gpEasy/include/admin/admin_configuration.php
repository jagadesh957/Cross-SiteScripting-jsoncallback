...
class admin_configuration{
	...
	function getValues(){
		...
		if( empty($show['Report_Errors']) ){
			$show['Report_Errors'] = true;
		}
...
*/
		...
		if( isset($_GET['gpreq']) && $_GET['gpreq'] == 'flush' ){
			echo '<input value="'.$langmessage['save'].'" type="submit" class="gppost gpsubmit" name="aaa" accesskey="s" />';
		}else{
	...
	function formInput($name,$value,$type='text'){
		...
		$len = min(40,$len);
		$text = '<input name="%s" size="%d" value="%s" type="'.$type.'"/>';
		echo '<div>';
