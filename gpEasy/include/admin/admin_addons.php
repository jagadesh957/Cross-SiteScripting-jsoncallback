...
*/
		...
		echo '<h2>';
		echo common::Link('Admin_Addons',$langmessage['add-ons']);
		echo ' » ';
		...
		if( isset($config['addons'][$addon]['editable_text']) && admin_tools::HasPermission('Admin_Theme_Content') ){
				...
				echo '<tr><td>';
					...
					echo '</td>';
					echo '<td>';
					echo common::Link('Admin_Theme_Content',$langmessage['edit'],'cmd=addontext&addon='.urlencode($addon),' title="'.urlencode($langmessage['editable_text']).'" name="admin_box" ');
					echo '</td></tr>';
		...
		if( count($available) == 0 ){
			...
			foreach($available as $folder => $info ){
				...
				if( function_exists('symlink') ){
					echo common::Link('Admin_Addons',$langmessage['develop'],'cmd=step1&mode=dev&source='.$folder);
				}
				echo '</td>';
	...
	function ShowInstalled(&$available){
		...
		foreach($show as $folder => $info){
			...
			echo ' &nbsp; ';
			echo common::Link('Admin_Addons',$langmessage['uninstall'],'cmd=uninstall&addon='.$folder,' name="admin_box"');
