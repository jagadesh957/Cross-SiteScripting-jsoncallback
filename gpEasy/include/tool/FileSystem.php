	 ...
	 function ReplaceDirs( $replace_dirs, &$clean_dirs ){
		...
		foreach( $replace_dirs as $to_rel => $from_rel ){
			...
			if( !$this->file_exists($to_full) ){
				$message = $langmessage['dir_replace_failed'].' (1)';
				break;
			}

			if( !$this->file_exists($from_full) ){
				$message = $langmessage['dir_replace_failed'].' (2)';
				break;
			}

			//rename the original to the trash
			if( !$this->rename( $to_full, $trash_full ) ){
				$message = $langmessage['dir_replace_failed'].' (3)';
				break;
			}
			...
			if( !$this->rename( $from_full, $to_full ) ){
				$message = $langmessage['dir_replace_failed'].' (4)';
				break;
			}
...
class gp_filesystem_ftp extends gp_filesystem_base{
	...
	function connectForm($args = false){
		...
		echo '<tr><td>';
			echo 'FTP Hostname';
			echo '</td><td>';
			echo '<input type="text" name="ftp_server" value="'.htmlspecialchars($args['ftp_server']).'" />';
			echo '</td></tr>';

		echo '<tr><td>';
			echo 'FTP Username';
			echo '</td><td>';
			echo '<input type="text" name="ftp_user" value="'.htmlspecialchars($args['ftp_user']).'" />';
			echo '</td></tr>';

		echo '<tr><td>';
			echo 'FTP Password';
			echo '</td><td>';
			echo '<input type="password" name="ftp_pass" value="" />';
			echo '</td></tr>';

		echo '<tr><td>';
			echo 'FTP Port';
			echo '</td><td>';
			echo '<input type="text" name="port" value="'.htmlspecialchars($args['port']).'" />';
			echo '</td></tr>';
	}
