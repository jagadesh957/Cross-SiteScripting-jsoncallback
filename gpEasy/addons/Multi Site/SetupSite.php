...
class SetupSite{
	...
	function Options_htaccess($site){
		...
		$site_url =& $this->siteData['sites'][$site]['url'];
	...
	function InstallLink($label,$query_array=array(),$attr=''){
		...
		return common::Link('Admin_Site_Setup',$label,$query,$attr);
	...
	function InstallFolder($destination){
		...
		$parent_dirs[] = $destination;
		while( $parent != '/' ){
			$parent_dirs[] = $parent;
			$parent = dirname($parent);
		}
		$parent_dirs = array_reverse($parent_dirs);

		echo '<div id="parent_folders">';
		foreach($parent_dirs as $dir){
			echo '<span>';
			$query_array = array('cmd'=>'expandfolder','folder'=>$dir);
			echo $this->InstallLink('/'.basename($dir),$query_array,' name="gpajax" rel="'.htmlspecialchars($dir).'" ');
			echo '</span>';
		}
		echo '</div>';
